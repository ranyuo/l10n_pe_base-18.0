...
class StockMovePleReport(models.TransientModel):
    ...
    def _get_ple_report_content(self, report):
        ...
        for _move_id, line_vals in groupby(lines_data, lambda line: line["move_id"]):
            ...
            if line.get('document_name_related',False) and line.get('document_type_related',False) and line.get('related_document_date',False):
                serie_folio = self._get_serie_folio(line.get('document_name_related',""))
                line["document_type"] = line.get('document_type_related',False)
                date = line["related_document_date"].strftime('%d/%m/%Y')
            else:
                ...
                #SI EXISTE PICKING_NAME ENTONCES ES UNA GUIA DE REMISION
                if delivery_number and line['picking_name']:
                    line['document_type'] = '09'
            ...
            values = {
                'period': period,
                'cuo': str(line['picking_id'] or count).replace('/', '').zfill(6),
    ...
    def _get_ple_reports_data(self):
        ...
        qu = query.select('stock_move.id',
                          ...
                          'case when stock_move__sol__solr__aml__am.l10n_pe_prefix_code is not null then concat(stock_move__sol__solr__aml__am.l10n_pe_prefix_code,\'-\',stock_move__sol__solr__aml__am.l10n_pe_invoice_number) else \'\' end AS move_name',
                          'case when stock_move__pol__aml_p__am_p.l10n_pe_prefix_code is not null then concat(stock_move__pol__aml_p__am_p.l10n_pe_prefix_code,\'-\',stock_move__pol__aml_p__am_p.l10n_pe_invoice_number) else \'\' end AS move_name_p',
                          'stock_move__picking.l10n_pe_edi_related_document_type AS document_type_related',
                          'stock_move__picking.l10n_pe_edi_document_number AS document_name_related',
                          'stock_move__picking.l10n_pe_edi_related_document_date AS related_document_date',
